...
const (
	FinancialService_ReportDelivery_FullMethodName = "/logistics.FinancialService/ReportDelivery"
)
...
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
//
// Servicio entre el Sistema Logístico y el sistema financiero Raquis
type FinancialServiceClient interface {
	// El sistema logístico reporta las entregas al sistema financiero Raquis
	ReportDelivery(ctx context.Context, in *FinancialReport, opts ...grpc.CallOption) (*emptypb.Empty, error)
}
...
func (c *financialServiceClient) ReportDelivery(ctx context.Context, in *FinancialReport, opts ...grpc.CallOption) (*emptypb.Empty, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(emptypb.Empty)
	err := c.cc.Invoke(ctx, FinancialService_ReportDelivery_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
type FinancialServiceServer interface {
	// El sistema logístico reporta las entregas al sistema financiero Raquis
	ReportDelivery(context.Context, *FinancialReport) (*emptypb.Empty, error)
...
func (UnimplementedFinancialServiceServer) ReportDelivery(context.Context, *FinancialReport) (*emptypb.Empty, error) {
...
func _FinancialService_ReportDelivery_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(FinancialReport)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(FinancialServiceServer).ReportDelivery(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: FinancialService_ReportDelivery_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(FinancialServiceServer).ReportDelivery(ctx, req.(*FinancialReport))
	}
	return interceptor(ctx, in, info, handler)
}
...
var FinancialService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "logistics.FinancialService",
	HandlerType: (*FinancialServiceServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "ReportDelivery",
			Handler:    _FinancialService_ReportDelivery_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "logistics.proto",
